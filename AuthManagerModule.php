 ...
 * 'controllerMap' => [
 *     'assignment' => [
 *         'class' => 'vthang87\authmanager\controllers\AssignmentController',
 ...
 * @since  1.0
 */
class AuthManagerModule extends \yii\base\Module{
	/**
	 * @inheritdoc
	 */
	 ...
	 * Its used when `layout` set to 'left-menu', 'right-menu' or 'top-menu'.
	 */
	public $mainLayout = '@vthang87/authmanager/views/layouts/main.php';
	/**
	 * @var array
	 * @see [[menus]]
	 */
	...
	public function init(){
		parent::init();
		if(!isset(Yii::$app->i18n->translations['authmanager'])){
			Yii::$app->i18n->translations['authmanager'] = [
				'class'          => 'yii\i18n\PhpMessageSource',
				'sourceLanguage' => 'en',
				'basePath'       => '@vthang87/authmanager/messages',
			];
		}
		...
		if($this->navbar === null && Yii::$app instanceof \yii\web\Application){
			$this->navbar = [
				['label' => Yii::t('authmanager','Help'),'url' => ['default/index']],
				['label' => Yii::t('authmanager','Application'),'url' => Yii::$app->homeUrl],
			];
		}
		if(class_exists('yii\jui\JuiAsset')){
			Yii::$container->set('vthang87\authmanager\AutocompleteAsset','yii\jui\JuiAsset');
		}
	}
		...
		if($this->_normalizeMenus === null){
			...
			foreach($this->_coreItems as $id => $lable){
				if(!isset($conditions[$id]) || $conditions[$id]){
					$this->_normalizeMenus[$id] = ['label' => Yii::t('authmanager',$lable),'url' => [$mid . $id]];
				}
			}
			foreach(array_keys($this->controllerMap) as $id){
				$this->_normalizeMenus[$id] = ['label' => Yii::t('authmanager',Inflector::humanize($id)),'url' => [$mid . $id]];
			}
	...
	public function beforeAction($action){
		if(parent::beforeAction($action)){
			...
			$view->params['breadcrumbs'][] = [
				'label' => ($this->defaultUrlLabel ?: Yii::t('authmanager','Admin')),
