...
for (const article of articles) {
    const relativeBox = article.getElementsByClassName('relative')[0];
    const iconCategory = relativeBox.getElementsByTagName('span')[0].className;
    const iconValue = article.getElementsByTagName('dd')[0].innerText;

    var iconClass = 'fa-' + iconValue;
    var iconName = iconValue;
    if (iconCategory.includes('fas')) {
        iconClass = 'fas ' + iconClass;
    }else if(iconCategory.includes('far')){
        iconClass = 'far ' + iconClass;
    }else if(iconCategory.includes('fab')){
        iconClass = 'fab ' + iconClass;
    }
    var icon = "'" +iconClass+"'=>'"+iconName+"'";
