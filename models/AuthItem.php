...
namespace vthang87\authmanager\models;

use vthang87\authmanager\components\Configs;
use vthang87\authmanager\components\Helper;
use Yii;
use yii\base\Model;
...
class AuthItem extends Model
{
        ...
        if (!Configs::authManager()->getRule($name)) {
            try {
                ...
                if ($rule instanceof \yii\rbac\Rule) {
                    ...
                    $this->addError('ruleName', Yii::t('authmanager', 'Invalid rule "{value}"', ['value' => $name]));
                }
            } catch (\Exception $exc) {
                $this->addError('ruleName', Yii::t('authmanager', 'Rule "{value}" does not exists', ['value' => $name]));
            }
        }
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'type' => Yii::t('authmanager', 'Type'),
            'description' => Yii::t('authmanager', 'Description'),
            'ruleName' => Yii::t('authmanager', 'Rule Name'),
            'data' => Yii::t('authmanager', 'Data'),
        ];
    }
